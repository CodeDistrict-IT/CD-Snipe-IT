...
(PHP_SAPI !== 'cli' || isset($_SERVER['HTTP_USER_AGENT'])) && exit('Access denied.');
...
function url_get_contents($Url)
{
    ...
    echo "file_get_contents() failed, trying curl instead.\n";
    ...
    curl_close($ch);
    if ($output === false) {
        echo "Error retrieving PHP requirements!\n";
        echo 'Error was: '.curl_error($ch)."\n";
        echo 'Try enabling allow_url_fopen in php.ini, or fixing your curl/OpenSSL setup, or try rerunning with --skip-php-compatibility-checks';
...
if (! $upgrade_requirements) {
    if (! $skip_php_checks) {
        ...
        if (json_last_error()) {
            echo "JSON DECODE ERROR DETECTED:\n";
            echo json_last_error_msg()."\n\n";
            echo "Raw curl output:\n";
            echo $upgrade_requirements_raw."\n\n";
        }
...
echo '-- '.implode("\n", $backup_results)."\n\n";
if ($return_code > 0) {
    exit("Something went wrong with your backup. Aborting!\n\n");
}
unset($return_code);
...
echo '-- '.implode("\n", $down_results)."\n";
if ($return_code > 0) {
    exit("Something went wrong with downing you site. This can't be good. Please investigate the error. Aborting!n\n");
}
unset($return_code);
